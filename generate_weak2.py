...
classes (appeal to emotion, fallacy of extension) using Groq API,
and merges them with existing augmented fallacies into fallacy_examples_augmented_v2.jsonl.
"""
...
def main():
    ...
    ap.add_argument("--base-data", default="data/processed/fallacies/fallacy_examples_augmented.jsonl")
    ap.add_argument("--output", default="data/processed/fallacies/fallacy_examples_augmented_v2.jsonl")
    args = ap.parse_args()
    ...
    print(f"\nSaved new synthetic topup: {len(new_df)} rows")

    # Load base dataset (fallacy_examples_augmented.jsonl or fallback to master)
    base_path = Path(args.base_data)
    if not base_path.exists():
        base_path = Path("data/processed/fallacies/fallacy_examples_master.jsonl")

    existing = pd.read_json(base_path, lines=True)
    if len(new_df) > 0:
        combined = pd.concat([existing, new_df[['text', 'fallacy_type', 'source', 'synthetic']]], ignore_index=True)
    else:
        combined = existing
    ...
    print(f"\nSaved combined augmented_v2 dataset: {len(combined)} total rows -> {out_path}")
    print(combined['fallacy_type'].value_counts())
