...
==================================================================================
Uses Playwright (headed Chromium) to render pages then passes the live HTML to
...
# Fixed slugs to match raw folder filenames exactly
GOV_SKIPPED = [
    ...
    {"url": "https://data.gov.in/sector/environment-forest-climate-change",
                                                             "slug": "sector_environment-forest-climate-change"},
    {"url": "https://data.gov.in/sector/finance",            "slug": "sector_finance"},
    {"url": "https://data.gov.in/sector/health-family-welfare",
                                                             "slug": "sector_health-family-welfare"},
    {"url": "https://data.gov.in/sector/labour-employment",  "slug": "sector_labour-employment"},
    {"url": "https://data.gov.in/sector/science-technology", "slug": "sector_science-technology"},
    {"url": "https://data.gov.in/sector/water-resources",    "slug": "sector_water-resources"},
]
...
def save_html(html: str, out_dir: Path, slug: str, url: str) -> bool:
    """Save rendered HTML and meta.json. Returns True if HTML is non-trivial."""
    if not html or len(html.strip()) < 1000 or "Access Denied" in html:
        log.warning("  Empty, small, or blocked HTML returned for %s", slug)
        return False
    ...
    meta = {
        ...
        "page_title": slug.replace("-", " ").replace("_", " ").title(),
        "status_code": 200,
    ...
    log.info("  [OK] %s - fetched %d bytes", slug, len(html))
    return True
...
def run_fetch(targets: list, out_dir: Path, source_name: str):
    ...
    log.info("=" * 60)
    log.info("  Fetching %d JS-rendered pages for: %s (HEADLESS=FALSE)", len(targets), source_name)
    log.info("=" * 60)
    ...
    with sync_playwright() as pw:
        # Using headless=False is critical to bypass Akamai bot detection on data.gov.in
